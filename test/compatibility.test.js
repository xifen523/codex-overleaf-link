...
test('release version metadata is aligned for v1.6.3 while native protocol stays unchanged', async () => {
...
test('buildBridgePingParams returns v1.3 extension protocol metadata', () => {
  assert.equal(compatibility.BUILD_TARGET_VERSION, '1.6.3');
  assert.equal(compatibility.EXTENSION_PROTOCOL_VERSION, 1);
  assert.deepEqual(compatibility.buildBridgePingParams({ version: '1.6.3' }), {
...
test('classifyNativeCompatibility returns compatible for minimum-or-newer protocol 1 hosts with all required capabilities', () => {
  const result = compatibility.evaluateNativeCompatibility(nativeResponse(), { version: '1.6.3' });

  assert.equal(compatibility.classifyNativeCompatibility(nativeResponse(), '1.6.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  assert.equal(result.requiredVersion, '1.0.0');
  assert.equal(result.recommendedVersion, '1.6.3');
  assert.equal(result.updateAvailable, false);
  assert.equal(result.updateCommand, canonicalInstallCommand('darwin'));
  assert.equal(result.releaseUrl, 'https://github.com/Ghqqqq/codex-overleaf-link/releases/tag/v1.6.3');
});
...
test('classifyNativeCompatibility keeps v1.0 protocol 1 hosts with required capabilities operational under v1.3', () => {
  ...
  const result = compatibility.evaluateNativeCompatibility(response, { version: '1.6.3' });

  assert.equal(compatibility.classifyNativeCompatibility(response, '1.6.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  ...
  assert.equal(result.updateCommand, compatibility.buildInstallCommand('1.6.3', 'darwin'));
