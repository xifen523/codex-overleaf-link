...
function statusFor(response, metadata = { version: '0.5.0' }) {
  return compatibility.evaluateNativeCompatibility(response, metadata).status;
}

function canonicalInstallCommand() {
  return 'CODEX_OVERLEAF_REF=v0.5.0 bash -c "$(curl -fsSL https://raw.githubusercontent.com/Ghqqqq/codex-overleaf-link/v0.5.0/install.sh)"';
}

test('buildBridgePingParams returns v0.5 extension protocol metadata', () => {
  assert.deepEqual(compatibility.buildBridgePingParams({ version: '0.5.0' }), {
...
test('evaluateNativeCompatibility classifies native versions below v0.5 as too old', () => {
  assert.equal(statusFor(compatibleNative({ version: '0.4.0' })), 'native_too_old');
});
...
test('evaluateNativeCompatibility classifies extension versions below native minimum as too old', () => {
  assert.equal(statusFor(compatibleNative({ minExtensionVersion: '0.6.0' })), 'extension_too_old');
});
