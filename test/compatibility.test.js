...
test('release version metadata is aligned for v1.1.3 while native protocol stays unchanged', async () => {
...
test('buildBridgePingParams returns v1.1 extension protocol metadata', () => {
  assert.equal(compatibility.BUILD_TARGET_VERSION, '1.1.3');
  assert.equal(compatibility.EXTENSION_PROTOCOL_VERSION, 1);
  assert.deepEqual(compatibility.buildBridgePingParams({ version: '1.1.3' }), {
...
test('classifyNativeCompatibility returns compatible for minimum-or-newer protocol 1 hosts with all required capabilities', () => {
  const result = compatibility.evaluateNativeCompatibility(nativeResponse(), { version: '1.1.3' });

  assert.equal(compatibility.classifyNativeCompatibility(nativeResponse(), '1.1.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  assert.equal(result.requiredVersion, '1.0.0');
  assert.equal(result.recommendedVersion, '1.1.3');
  assert.equal(result.updateAvailable, false);
  assert.equal(result.updateCommand, canonicalInstallCommand('darwin'));
  assert.equal(result.releaseUrl, 'https://github.com/Ghqqqq/codex-overleaf-link/releases/tag/v1.1.3');
});
...
test('classifyNativeCompatibility keeps v1.0 protocol 1 hosts with required capabilities operational under v1.1', () => {
  ...
  const result = compatibility.evaluateNativeCompatibility(response, { version: '1.1.3' });

  assert.equal(compatibility.classifyNativeCompatibility(response, '1.1.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  ...
  assert.equal(result.updateCommand, compatibility.buildInstallCommand('1.1.3', 'darwin'));
...
test('classifyNativeCompatibility returns update-available for older protocol 1 hosts with the allowed-method capability subset', () => {
  ...
  assert.equal(compatibility.classifyNativeCompatibility(response.result, '1.1.3'), 'update-available');
...
test('classifyNativeCompatibility returns incompatible for missing native, unsupported protocol, extension mismatch, or unhealthy local Codex', () => {
  assert.equal(compatibility.classifyNativeCompatibility(null, '1.1.3'), 'incompatible');
  assert.equal(
  ...
  assert.equal(
    compatibility.evaluateNativeCompatibility(nativeResponse({ minExtensionVersion: '1.1.4' }), { version: '1.1.3' }).status,
    'extension_too_old'
  );
  assert.equal(
...
test('buildInstallCommand returns release-pinned npm native update commands', () => {
  assert.equal(
    compatibility.buildInstallCommand('1.1.3', 'darwin'),
    'npm exec --yes codex-overleaf-link@1.1.3 -- install-native'
  );
  assert.equal(
    compatibility.buildInstallCommand('v1.1.3', 'linux'),
    'npm exec --yes codex-overleaf-link@1.1.3 -- install-native'
  );
  assert.equal(
    compatibility.buildInstallCommand('1.1.3', 'win32'),
    'npm exec --yes codex-overleaf-link@1.1.3 -- install-native'
  );
});
...
test('buildInstallCommand omits bundled id but embeds a custom extension id', () => {
  ...
  assert.equal(
    compatibility.buildInstallCommand('1.1.3', 'darwin', bundledExtensionId),
    'npm exec --yes codex-overleaf-link@1.1.3 -- install-native'
  );
  assert.equal(
    compatibility.buildInstallCommand('1.1.3', 'darwin', extensionId),
    `npm exec --yes codex-overleaf-link@1.1.3 -- install-native --extension-id ${extensionId}`
  );
  assert.equal(
    compatibility.buildInstallCommand('1.1.3', 'win32', extensionId),
    `npm exec --yes codex-overleaf-link@1.1.3 -- install-native --extension-id ${extensionId}`
  );
});
