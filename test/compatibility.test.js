...
function statusFor(response, metadata = { version: '0.4.0' }) {
  return compatibility.evaluateNativeCompatibility(response, metadata).status;
}

function canonicalInstallCommand() {
  return 'CODEX_OVERLEAF_REF=v0.4.0 bash -c "$(curl -fsSL https://raw.githubusercontent.com/Ghqqqq/codex-overleaf-link/v0.4.0/install.sh)"';
}
...
test('evaluateNativeCompatibility classifies native versions below v0.4 as too old', () => {
  assert.equal(statusFor(compatibleNative({ version: '0.3.0' })), 'native_too_old');
});
...
test('evaluateNativeCompatibility classifies extension versions below native minimum as too old', () => {
  assert.equal(statusFor(compatibleNative({ minExtensionVersion: '0.5.0' })), 'extension_too_old');
});
