...
test('release metadata is prepared for v1.3.7', () => {
  assert.equal(packageJson.version, '1.3.7');
  assert.equal(extensionManifest.version, packageJson.version);
});

test('release docs carry exact v1.3.7 badge and changelog heading', () => {
  ...
  assert.match(changelog, new RegExp(`^## v${escapedVersion} - 2026-05-23$`, 'm'));
  assert.doesNotMatch(changelog, new RegExp(`^## \\[${escapedVersion}\\] - 2026-05-23$`, 'm'));
