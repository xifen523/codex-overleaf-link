...
test('release metadata is prepared for v1.6.2', () => {
  assert.equal(packageJson.version, '1.6.2');
  assert.equal(extensionManifest.version, packageJson.version);
});

test('release docs carry exact v1.6.2 badge and changelog heading', () => {
  ...
  assert.match(changelog, new RegExp(`^## v${escapedVersion} - 2026-06-17$`, 'm'));
  assert.doesNotMatch(changelog, new RegExp(`^## \\[${escapedVersion}\\] - 2026-06-17$`, 'm'));
