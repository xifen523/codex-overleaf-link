...
test('release metadata is prepared for v2.0.0', () => {
  assert.equal(packageJson.version, '2.0.0');
  assert.equal(extensionManifest.version, packageJson.version);
});

test('release docs carry exact v2.0.0 badge and changelog heading', () => {
  ...
  assert.match(changelog, new RegExp(`^## v${escapedVersion} - 2026-07-14$`, 'm'));
  assert.doesNotMatch(changelog, new RegExp(`^## \\[${escapedVersion}\\] - 2026-07-14$`, 'm'));
