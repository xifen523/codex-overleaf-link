...
test('release metadata is prepared for v1.4.0', () => {
  assert.equal(packageJson.version, '1.4.0');
  assert.equal(extensionManifest.version, packageJson.version);
});

test('release docs carry exact v1.4.0 badge and changelog heading', () => {
  ...
  assert.match(changelog, new RegExp(`^## v${escapedVersion} - 2026-05-31$`, 'm'));
  assert.doesNotMatch(changelog, new RegExp(`^## \\[${escapedVersion}\\] - 2026-05-31$`, 'm'));
